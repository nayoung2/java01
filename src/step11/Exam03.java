...
public class Exam03 {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(plus(10, 20f));         // plus(float,float) //float -> int 로는 암시적 형변환이 안되지만 int -> float 형변환 가능 => 10이 float로 암시적 형변환됨
    System.out.println(plus(10f, 20));         // plus(float,int)

    //암시적 형변환까지 동원하여 형식에 맞는 메소드를 찾아 실행, 암시적 형변환이 불가능한 경우 오류
    //(일치하는 메소드가 없으면 형변환 하지 X, 일치하는 메소드가 없으면 암시적 형변환 동원, 그래도 일치하는 메소드가 없으면 오류)
...
- 의미?
  ...
  (개발자가 plus는 어떤식으로 계산되는지(변수type 등), plus2는 어떤식으로 계산되는지 다 암기하기 너무 힘들다.)

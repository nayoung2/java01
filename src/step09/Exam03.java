...
public class Exam03 {
  ...
  static class Score {
    ...
    // 생성자를 만들지 않으면 다음과 같이 기본 생성자가 자동으로 추가된다. -> 자바의 모든 클래스는 생성자를 가진다.
    // public Score() {}

    // 다음과 같이 개발자가 직접 기본 생성자를 정의할 수 있다.
    Score() {   // this가 숨어잇움
      System.out.println("생성자 호출됨!"); //만약 생성자가 호출된다면 이 문장이 출력된다.
    }
  }
...
- 인스턴스가 사용되기 전에 유효한 값으로 초기화하기 위해서. * 즁요!
  => 예) 냉장고 초기 냉동실 온도와 냉장실 온도
  => 예) 모니터 밝기 및 명암이 유효한 초기값으로 셋팅되어 있다.
  => 일상 생활에서 물건은 사용하기 전에 적절한 초기값으로 설정하는 것이
    일반적이다.
- 모든 클래스는 반드시 한 개 이상의 생성자가 있어야 한다.
- 개발자가 생성자를 만들지 않으면, 컴파일러가 기본 생성자를 자동으로 붙인다.
(Exam02에서도 Score class 정의할 때 자동으로 붙음)

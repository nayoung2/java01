...
- 주요 메서드
1) toString() : 클래스이름@해시값  을 리턴한다.
2) hashCode() : 인스턴스를 구별하기 위한 4바이트 정수 값. 주소 아님! (인스턴스를 식별하는 유일한 식별자)  ....hash : 디지털 지문....
3) equals() : 인스턴스가 같은지 비교
...
6) finalize() :
    ...
    => 보통 가비지 컬렉터의 실행 시점을 개발자가 임의적으로 조정하지 않기 때문에
     ...
     따라서 finalize의 호출 시점을 명확히 알 수 없다.
     메모리가 부족하지 않다면 프로그램이 종료될 때 까지 가비지 컬렉터가
     동작하지 않는다. => 프로그램이 종료될 때 까지 finalize()가 호출되지
     않을 수 있다. --> finalize()에 의존하는 코드를 작성하지 말라!
     --> 오버라이딩하지 말라! C++의 소멸자와 다르다.
*/
